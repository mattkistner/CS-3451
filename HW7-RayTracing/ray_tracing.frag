...
Hit hitSphere(const Ray r, const Sphere s) 
{
    ...
    float B = -2.0 * dot((s.ori - r.ori), r.dir);
    float C = dot((s.ori - r.ori), (s.ori - r.ori)) - dot(s.r, s.r);
...
vec3 shadingPhong(Light light, int matId, vec3 e, vec3 p, vec3 s, vec3 n) 
{
    ...
    vec3 dcolor = matId == 0 ? vec3(0.2, 0, 0) : vec3(0, 0, 0.3);
    ...
    vec3 lightD = normalize(s - p);
    vec3 view = normalize(e - p);
    vec3 reflection = reflect(-lightD, n);

    vec3 ambient = ka * light.Ia;
    vec3 diffuse = kd * light.Id * max(dot(lightD, n), 0.0);
    vec3 specular = ks * light.Is * pow(max(dot(reflection, view), 0.0), shininess);

    vec3 phong = ambient + diffuse + specular;
    vec3 color = mix(dcolor, phong, 1.0);
...
vec3 sampleDiffuse(int matId, vec3 p) 
{
    ...
    if(matId == 0) {		
        ...
        color = mat_color * texture(floor_color, uv).rgb;
...
bool isShadowed(Light light, Hit h) 
{
    ...
    Ray shadowedRay;
    shadowedRay.ori = intersect + 0.001 * dir;
    shadowedRay.dir = dir;

    Hit shadowedHit = findHit(shadowedRay);
    if (shadowedHit.t > 0.0 && shadowedHit.t < t_max) {
        shadowed = true;
    }
...
void main() 
{
    ...
    for(int i = 0; i < recursiveDepth; i++) {
        ...
        recursiveRay.ori = hit.p + 0.001 * reflected_dir;
