...
public:
	...
	void Create_Angry_Bird_Palace()
	{
		...
		for (int i = 0; i < bird_num; i++) {
			...
			{ 
				float x = -5 + (5 * time[i]);
				float y = (9.8 * time[i]) - (0.5 * ((9.8) * (time[i] * time[i])));
				float angle = ((-150) * (3.141592/180)) * time[i];
				Matrix4f t;
				t << 1., 0., 0., x,
					0., 1., 0., y,
					0., 0., 1., 0.,
	...
	void Create_Angry_Bird_Garden()
	{
		...
		// My silly lil building
		auto building = Add_Obj_Mesh_Object_From_File("building1.obj", OpenGLColor(.78f, .55f, .01f, 1.f));
		{
			Matrix4f t;
			t << 5., 0., 0., -5.,
				0., 5., 0., 1.2,
			...
			building->Set_Model_Matrix(t*r);
		}

		// yay pretty tree woohoo
		for (int i = 0; i < 5; i++) {
			auto tree = Add_Obj_Mesh_Object_From_File("tree2.obj", OpenGLColor(0.f, 1.f, 0.f, 1.f));
			{
				Matrix4f t;
		...
		//the fountain
		auto fountain = Add_Obj_Mesh_Object_From_File("fountain1.obj", OpenGLColor(.01f, .52f, .66f, 1.f));
		{
			Matrix4f t;
			t << 2., 0., 0., 5.,
				0., 2., 0., 1.0,
			...
			fountain->Set_Model_Matrix(t);
		}

		// circle of grass
		int grass_num = 24;
		for (int i = 0; i < grass_num; i++) {
			auto grass = Add_Obj_Mesh_Object_From_File("grass2.obj", OpenGLColor(0.f, 1.f, 0.f, 1.f));
			{
				float angle = i * (15 * (3.141592/180));
				Matrix4f t;
				t << 1., 0., 0., 8 * cos(angle) ,
