...
vec4 shading_texture_with_checkerboard() 
{
    ...
    vec2 cSize = vec2(10.0);
    vec2 ix = floor(cSize * vtx_uv.x);
    vec2 iy = floor(cSize * vtx_uv.y);

    if (mod(int(ix) + int(iy), 2.) == 1) {
        color = vec3(0.0);
    } else {
        color = vec3(1.0);
    }
...
vec4 shading_texture_with_phong(Light light, vec3 e, vec3 p, vec3 s, vec3 n) 
{
    ...
    vec3 phong = (ka * light.Ia) + (kd * light.Id * max(dot(lightDir, n), 0.0)) 
        + (ks * light.Is * pow(max(dot(view, reflect(-lightDir, n)), 0.0), shininess));
    vec3 phong_colors = phong * tex_color;
    color = vec4(phong_colors, 1.0);
...
vec3 read_normal_texture() 
{
    ...
    normal = normalize(texture(tex_normal, vtx_uv).rgb * 2.0 - 1.0);
...
vec4 shading_texture_with_normal_mapping()
{
    ...
    N = read_normal_texture();
    perturbed_normal = calc_perturbed_normal(TBN, N);
